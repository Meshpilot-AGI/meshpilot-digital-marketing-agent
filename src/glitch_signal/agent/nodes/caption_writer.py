...
from glitch_signal.shared_context import resolve_brand_display_name
...
async def _generate_caption(
    ...
    display_name = await resolve_brand_display_name(brand_id)
    if not display_name:
...
async def _generate_via_vision(
    *,
    ...
    """
    # NOTE (GROW-BEDROCK policy exception): this captions from the VIDEO
    # itself. Amazon Bedrock / Claude has no video-understanding modality,
    # so this path legitimately stays on Gemini — it is an "AWS doesn't have
    # the model" exception, not an un-migrated LLM call. Text LLM is on Bedrock.
