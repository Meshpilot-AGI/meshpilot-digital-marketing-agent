...
from glitch_signal.shared_context import resolve_publish_platform_hub_first
...
async def drive_scout_node(state: SignalAgentState) -> SignalAgentState:
    ...
    # specific re-runs).
    # Hub-first platform resolution (R1 shared-access boundary).
    # If the hub confirms the brand has a connected platform account,
    # use the vendor key from local config. Otherwise fall back to
    # legacy standalone resolution.
    hub_platform = await resolve_publish_platform_hub_first(brand_id, "tiktok")
    if hub_platform:
        default_platform = hub_platform
        log.info("drive_scout.hub_platform_chosen", brand=brand_id, platform=default_platform)
    else:
        try:
