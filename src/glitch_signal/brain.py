...
shared `glitch-brain-mcp` so sibling agents on the same brand can
...
Wiring contract (BIND-1b multi-brand pattern):
  ...
  - Per-brand tokens: `BRAIN_TOKEN_BSK_004_<BRAND_SLUG_UPPER>` (e.g.
    `BRAIN_TOKEN_BSK_004_GLITCH_EXECUTOR`, `BRAIN_TOKEN_BSK_004_AYURPET`).
...
Social is enrolled in `glitch-executor` and `ayurpet` — both have
...
(`ayurpet`). The normalizer accepts both; the env-key suffix is
always UPPER_SNAKE.
"""
...
# The brain telemetry mirror is OPTIONAL. `meshpilot_platform` lives in the
# original Mesh Pilot monorepo and is not bundled into this standalone
# service. When it is absent we degrade to a no-op: nothing is mirrored and
# `brain_available*()` report False, so the scheduler simply skips the mirror.
try:
    from meshpilot_platform.brain import BrainAuthError, BrainClient, BrainError
    from meshpilot_platform.brain.limits import cap_payload

    _BRAIN_SDK_AVAILABLE = True
except ModuleNotFoundError:  # pragma: no cover - exercised only in bundled mode
    _BRAIN_SDK_AVAILABLE = False

    class BrainError(Exception):
        """Fallback so `except BrainError` clauses still resolve."""

    class BrainAuthError(BrainError):
        """Fallback auth-error stand-in."""

    class BrainClient:  # noqa: D401 - inert placeholder, never instantiated
        """Inert placeholder; never used when the SDK is missing."""

    def cap_payload(payload, *_args, **_kwargs):
        return payload
...
def brain_available_for(brand_id: str | None) -> bool:
    """True when a BSK-004 token resolves for this brand AND the brain SDK
    is bundled. In this standalone service the SDK is absent, so this is
    always False unless `meshpilot_platform` is installed alongside."""
    if not _BRAIN_SDK_AVAILABLE:
        return False
...
def brain_available() -> bool:
    ...
    legacy bare key) AND the brain SDK is bundled. Used as a global
