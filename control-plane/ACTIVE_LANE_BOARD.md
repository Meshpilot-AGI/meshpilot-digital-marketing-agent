...
Shipped: JOBS-0 design (#307) · JOBS-1 schema+gates (#308) · JOBS-2 discovery (#309) · JOBS-3 scoring (#310) · JOBS-4 tailoring+verifier (#311) · JOBS-5 approval cards (#312) · JOBS-6 submission guards (#313) · JOBS-4b render_cv (#314) · JOBS-7 Railway submitter.
...
2. ✅ RESOLVED by JOBS-7 — the submitter is a SECOND RAILWAY SERVICE (`submitter/`), not FastAPI
   Cloud. Playwright's own image ships Chromium + system deps. Still needs deploying + a live
   dry-run. Original constraint, for the record: **neither Greenhouse nor Lever
...
- **JOBS-ROUTER-COST — cost-first model routing on the `complex` tier** (2026-09-15) — the operator asked for cheapest-first; a first attempt was reverted on a 0/18 measurement that turned out to be measuring OUR bug: the empty-completion retry was gated out at exactly the 8000 the scorer asks for, and nothing capped `reasoning.effort`. Both fixed; `complex` now runs **z-ai/glm-5.3 → sonnet-5**, and scoring sends `effort=low`. Measured **$0.0027 vs $0.0105 (74% cheaper)** on the same call; **17/18 scored, Spearman 0.914** against the stored sonnet run; suite 1351 pass. ⚠️ glm scores **+0.37 higher on average, none lower** — against the fixed **4.0 floor** that loosens the gate (1 → 2 roles at the floor on an unchanged pool). `critical` stays quality-first; `moderate`/`simple` left unmeasured, deliberately. → supervisor
