...
**Outcome — shipped the revoke, DEFERRED the pgvector move (two CI failures taught why):**
- PR #140 (naive `revoke`) failed the from-scratch CI gate: `rls_auto_enable()` isn't in these migrations (out-of-band Supabase fn), so the throwaway DB lacks it. → guarded it (function/role existence checks).
- PR #141 (guarded revoke **+ extension move**) failed the CI **idempotency re-apply**: once `alter extension vector set schema extensions` runs, the earlier `agent_memory` HNSW-index migration re-applies with an UNQUALIFIED `halfvec_cosine_ops`, which the CI's `psql` (search_path = `public`) can't resolve → `operator class … does not exist`. Prod's `postgres` role has `extensions` in search_path so it's fine there, but the CI (and any future consumer without that path) is not.
- PR #142 (SUPA-HARDEN-REVOKE) = **revoke only**, guarded + idempotent → CI green.

Neither #140 nor #141 ever reached prod (the integration applies only after CI is green), so prod stayed clean throughout. Revoke re-verified on the live DB via `begin; <guarded body>; select …; rollback;` → in-txn `rls_auto_enable` grantees dropped to `postgres, service_role` (PUBLIC/anon/authenticated gone). No app code changed.

**Remains (two deferred items, both documented):**
- **pgvector move (lint 0014):** relocating `vector` out of `public` needs a coordinated change — create it in `extensions` from `init_schema` AND put `extensions` on every consumer search_path (or schema-qualify every opclass reference in migrations) — else the CI idempotency gate breaks as above. "Extension in public" is benign + Supabase-default; low value, deferred to its own lane.
- **⚠️ `ensure_rls`/`rls_auto_enable` not in migrations:** the auto-RLS trigger lives only on prod. A from-scratch rebuild would NOT reproduce RLS on the ~13 tables that rely on it (they'd be RLS-off). The more important of the two — worth a lane to codify the trigger into a migration so from-scratch == prod.
